	...
	public static SessionFactory getSessionFactory() {
		if (sessionFactory == null) {
			try {
				...
				settings.put(Environment.URL, "jdbc:mysql://localhost/parser?useSSL=false");
				settings.put(Environment.USER, "root");
				settings.put(Environment.PASS, "tooor");
				settings.put(Environment.DIALECT, "org.hibernate.dialect.MySQL5Dialect");
				settings.put(Environment.SHOW_SQL, "true");
				settings.put(Environment.CURRENT_SESSION_CONTEXT_CLASS, "thread");
				settings.put(Environment.HBM2DDL_AUTO, "update");
				configuration.setProperties(settings);
				configuration.addAnnotatedClass(Trace.class);
				configuration.addAnnotatedClass(LogFile.class);
				configuration.addAnnotatedClass(Contrat.class);
				configuration.addAnnotatedClass(User.class);
				configuration.addAnnotatedClass(Occurrence.class);
